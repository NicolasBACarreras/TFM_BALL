...
from filter_scHiC import get_cells, cis_ratio
import pandas as pd
...
for index, row in translocation_file_pd.iterrows():
    ...
    list_of_translocations.append(translocation)
    ...
    trans_reso=1_000_000
    translocation_contact_finder(adts, chr1, start1, end1, chr2, start2, end2, trans_reso, cells2, translocation) 
    
    for cell, features in all_cells.items():
        try:
            adts.obs.loc[cell, translocation] = adts.obs.loc[cell, translocation] / adts.obs.loc[cell, "trans"]
        except KeyError:
            continue
    sc.pl.umap(adts, color=['leiden', 'group', translocation], size=200, save=f"_{translocation}_embedding.svg")
...
for translocation in list_of_translocations:


    data = adts.obs[['leiden', translocation]]
    ...
    sns.boxplot(x='leiden', y=translocation, data=df)
    plt.title(f'{translocation} by Cluster')
    plt.savefig(f'{outdir}/ball_{translocation}_by_cluster.svg')
    plt.show()

    # Statistical test (t-test in this case, assuming normal distribution)
    group1 = df[df['leiden'] == '0'][translocation]
    group2 = df[df['leiden'] == '1'][translocation]
